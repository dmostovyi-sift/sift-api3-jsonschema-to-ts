...
export function transformJavaTypesToJsonSchema(referencesMap, file) {
  const currentDir = file ? (file.dirpath === "." ? "" : file.dirpath) : "";
  ...
  function relativizeRefs(node) {
    ...
    if ("$ref" in node) {
      ...
      if (!hash && relativeFilePath === file.name + ".json") {
        ...
        node.$ref = hash ? `${relativeFilePath}#${hash}` : relativeFilePath;
      }
    }
    for (const k in node) {
  ...
  return function (obj, key) {
    if (key === "javaType") {
      ...
      key === "type" &&
      typeof obj[key] === "string" &&
      !JSON_SCHEMA_TYPES.has(obj[key])
    ) {
      // type field holds a Java type (e.g. double, long, java.util.Map<String, Foo>)
      // — process it the same way as javaType
      const schema = javaTypeToJsonScheme(obj[key], referencesMap);
      if (file) relativizeRefs(schema);
      Object.assign(obj, schema);
      // schema sets type to the appropriate JSON Schema type, overwriting the Java string
    }
  };
}
