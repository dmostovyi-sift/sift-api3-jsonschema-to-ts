...
export function transformJavaTypesToJsonSchema(referencesMap, file) {
  ...
  return function (obj, key) {
    if (key === "javaType") {
      ...
      key === "type" &&
      typeof obj[key] === "string" &&
      (obj[key].startsWith("java.") || obj[key].includes("<"))
    ) {
      // type field holds a Java collection type (e.g. java.util.Map<String, Foo>)
      // — process it the same way as javaType
      const schema = javaTypeToJsonScheme(obj[key], referencesMap);
      if (file) relativizeRefs(schema);
      Object.assign(obj, schema);
      // schema sets type: "object", so the Java string is overwritten
    }
  };
}
