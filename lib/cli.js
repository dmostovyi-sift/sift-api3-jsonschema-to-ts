...
export async function compile(input, output, { onProgress } = {}) {
  ...
  for (const file of files) {
    FileTransformer.fromFile(file)
      ...
      .map(collapseArrayTypeReferences())
      .traverse(transformJavaTypesToJsonSchema(referenceCollection, file))
  ...
  for await (const file of files) {
    current++;
    onProgress?.(current, total, file.name + ".yaml");
...
function buildReferencesMap(files) {
  ...
  for (const file of files) {
    const dirPrefix = file.dirpath === "." ? "" : file.dirpath + "/";
    referencesMap.set(
      javaTypeToModuleName(file.json.javaType).module,
      dirPrefix + file.name + ".json",
    );
    ...
    for (const nestedRef of nestedRefs) {
      ...
      referencesMap.set(
        ...
        `${dirPrefix}${file.name}.json#/properties/${path}`,
      );
    }
  }
