...
export const TYPES_TEMP_DIR = resolveHomeDir("~/console/src/schema/");
...
export function saveType(data, filePath) {
  ...
  fs.writeFileSync(
    path.resolve(TYPES_TEMP_DIR, filePath.replace("yaml", "ts")),
    data,
    { encoding: "utf8" },
  );
}
...
export function saveJson(data, filePath) {
  ...
  fs.writeFileSync(
    path.resolve(JSON_TEMP_DIR, filePath.replace("yaml", "json")),
